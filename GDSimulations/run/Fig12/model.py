...
class Simulator:
    def __init__(self, d, k, kstar, alpha, rep, eta, Tmax, lam, gamma, delta, num_save, data, saveZ, saveLabels, ID):
        """
        ...
        self.Nn = len(self.alpha)  # all alpha lists should have the same length
        ...
        self.Teacher = generate_Wishart((self.Nn, self.rep, self.d, self.ms))  # shape Nn x rep x d x d
    ...
    def simulate_one(self, n, Teacher):
        ...
        runtime = time() - t0
        print('Run Finished in {0} sec, MSE = {1}, Loss = {2}'.format(round(runtime, 2), torch.mean(MSE[-1]), torch.mean(Loss[-1])))
        print("")
